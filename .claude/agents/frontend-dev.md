...
frontend/src/
  ...
  utils/
    labelConfig.ts                 # Общие маппинги: SOFTWARE_LABEL_STYLES, SOFTWARE_LABEL_LIST, REPORT_TYPE_STYLES, REPORT_TYPE_RECOMMENDED_LABELS
  api/
  ...
  components/
    layout/
      ...
      SettingsSidebar.tsx           # Навигация по настройкам
      SidebarLink.tsx              # Общий компонент ссылки сайдбара (▶/◇ декоратор)
    common/
      Tag.tsx                      # Базовый тег/бейдж: size xs|sm|default + colorClass
      PageHeader.tsx               # Заголовок страницы: title + опциональный subtitle
      EmptyState.tsx               # Пустое состояние: "// сообщение"
      ModalShell.tsx               # Обёртка модалки: ›_ header, backdrop, actions, fullHeight
      ConfirmModal.tsx             # Диалог подтверждения (на основе ModalShell)
      SeverityBadge.tsx            # Бейдж критичности (на основе Tag)
      AutomationBadge.tsx          # Бейдж автоматизации (на основе Tag)
      RichEditor.tsx               # TipTap-обёртка (вставка изображений с авто-подписью)
...
- **Теги/бейджи** → `<Tag colorClass="..." size="xs|sm|default">` — ЗАПРЕЩЕНО писать `font-mono text-[Npx] tracking-widest px-1.5 py-0.5 border` вручную
- **Заголовки страниц** → `<PageHeader title="..." subtitle="..." />` — ЗАПРЕЩЕНО писать `<h1 className="font-display text-2xl...">` вручную
- **Пустые состояния** → `<EmptyState message="..." />` — для пустых таблиц/списков
- **Модалки** → `<ModalShell open title onClose actions>` — ЗАПРЕЩЕНО дублировать `dialog.modal-open` + backdrop вручную. Исключения: AIGenerateModal, CVSSCalculatorModal (кастомный layout)
- **Ссылки сайдбара** → `<SidebarLink to label delay />` — для навигации в сайдбарах
- **Маппинги лейблов** → импорт из `utils/labelConfig.ts` — ЗАПРЕЩЕНО определять маппинги стилей лейблов/типов inline
- **Секционные заголовки** → CSS-класс `.label-section` — моноширинный uppercase 10px
...
**Типографика:**
- Заголовки страниц: через `<PageHeader>` (Chakra Petch)
- Метки секций / заголовки таблиц: CSS-класс `.label-section`
- Коды / ID / бейджи: `font-mono` (JetBrains Mono), обёрнуты в `<Tag>`
...
- Inline raw-классы для тегов/бейджей — использовать компонент `Tag`
- Создание модалок без `ModalShell` — кроме AIGenerateModal и CVSSCalculatorModal
- Дублирование маппингов цветов лейблов — импортировать из `utils/labelConfig.ts`
- Ручной `<h1>` для заголовков страниц — использовать `<PageHeader>`
